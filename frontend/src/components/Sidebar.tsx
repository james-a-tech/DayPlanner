          ...
          <nav className="space-y-2">
            {menuItems.map((item) => (
              <a
                key={item.label}
                href={item.href}
                className={`flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-gray-100 text-gray-700 font-medium ${activeTab === item.tab ? 'bg-gray-100' : ''}`}
                onClick={item.tab && setActiveTab ? (() => { setActiveTab(item.tab); }) : undefined}
              >
